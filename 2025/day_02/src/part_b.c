...
int main(int argc, char *argv[argc + 1]) {
  ...
  for (size_t i = 0; i < input.size; i++) {
    for (size_t target_num = input.items[i].start;
      ...
      for (size_t parts = 2; parts <= end; parts++) {
        if (end % parts != 0) {
          continue;
        }
        bool equal = true;
        size_t limit = end / parts;
        size_t offset = end / parts;
        for (size_t left = 0; left < limit; left++) {
          for (size_t current_part = 1; current_part < parts && equal;
               current_part++) {
            equal =
                target_str[left] == target_str[(current_part * offset) + left];
          }
        }
        if (equal) {
