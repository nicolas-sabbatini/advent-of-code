...
fn main() {
    ...
    loop {
        ...
        for n in neighbors.iter() {
            if n.to_string().chars().all(|c| c.is_uppercase()) {
                ...
                if (!already_in || (already_in && !twice)) && *n != "start" {
                    paths_buildier.push((n, curren, already_in || twice));
                    need_to_add.push(paths_buildier.len() - 1);
                }
            }
        }
        if need_to_add.len() <= 0 {
            break;
        }
    }
...
fn build_path<'a>(p: &Vec<(&'a str, usize, bool)>, index: usize) -> (Vec<&'a str>, bool) {
